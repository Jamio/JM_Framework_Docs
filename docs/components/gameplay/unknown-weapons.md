...
Unknown Weapons makes firearms outside the mission's approved equipment pool less accurate and more likely to malfunction. Players can still use captured weapons, but issued and familiar equipment remains the safer choice.

Known weapons are taken from the framework's Arsenal presets and role loadouts, so a separate weapon list is not required. The feature uses ACE overheating and jamming and can be enabled or adjusted through Optional Systems.
