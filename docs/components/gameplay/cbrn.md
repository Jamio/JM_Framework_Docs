...
CBRN adds configurable chemical, biological and radiological threats to a mission. Its main gameplay loop is detection, protection, diagnosis, treatment and decontamination. Players need to identify dangerous areas, wear suitable PPE and seek medical help if they become exposed. The system uses ACE interactions and medical actions, with extra effects available when KAT is loaded.

Threats can be created as zones or contaminated source objects. Mission-makers choose the threat type and subtype, exposure strength, radius, required protection and particle effect. Biological threats can spread between players, while the custom-agent settings can be used to create a mission-specific disease or contaminant.

Players can use the CBRN detector to check the environment and receive vague symptoms when checking themselves. Medics can use the bioassay to find the exact threat, its severity and the required treatment. Untreated exposure can progress from minor visual effects and pain to collapse, unconsciousness, vomiting and death.

Treatment deals with the medical effects of exposure. Decontamination removes contamination from people, bodies, vehicles and containers. Depending on the object, this may involve a personal kit, shower, cleaning wipes, foam sealant, lined container or CBRN body bag. ZEN tools are also included for adding, removing and inspecting contamination during testing or a live mission.
